    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
        {stats.map(({ icon: Icon, label, value, color, bg }) => (
          <div key={label} className="bg-dark-900 border border-dark-800 rounded-xl p-6 flex items-center gap-5">
            ...
            {recent.map(car => (
              <div key={car._id} className="px-6 py-4 flex items-center justify-between hover:bg-dark-800/50 transition-colors">
                ...
                <div className="flex items-center gap-4 ml-4">
                  ...
                  <Link href={`/admin/cars/${car._id}/edit`} className="text-xs text-dark-400 hover:text-gold-400 transition-colors border border-dark-700 hover:border-gold-500/50 px-3 py-1.5 rounded-lg">
                    Edit
                  </Link>
                </div>
              </div>

...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Action
    ref={ref}
    className={cn('inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-dark-600 bg-transparent px-3 text-sm font-medium transition-colors hover:bg-dark-800 focus:outline-none focus:ring-2 focus:ring-gold-500 disabled:pointer-events-none disabled:opacity-50', className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn('absolute right-2 top-2 rounded-md p-1 text-dark-400 opacity-0 transition-opacity hover:text-white focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100', className)}
    toast-close=""
    {...props}
  >
    <X className="h-4 w-4" />
